...
const BillDetail = ({ currencyCodeList }: BillDetailProps) => {
  // 제네릭 타입을 BillOutgoingFormInput으로 설정하여 타입 안정성 확보
  const { control, setValue, getValues } = useFormContext<BillOutgoingFormInput>();
  ...
  useEffect(() => {
    ...
    if (lastModifiedCurrency.current === 'NONE') {
      ...
      const totalForeign = foreignGovFee + foreignAgencyFee + foreignVat + foreignEtcFee;
      setValue("foreignCostAmount", totalForeign);
      return;
    }
    ...
    if (lastModifiedCurrency.current === 'KRW') {
      const syncToForeign = (krwVal: number, foreignFieldName: keyof BillOutgoingFormInput) => {
      ...
      const syncToKRW = (foreignVal: number, krwFieldName: keyof BillOutgoingFormInput) => {
    ...
    const totalForeign = foreignGovFee + foreignAgencyFee + foreignVat + foreignEtcFee;
    setValue("foreignCostAmount", totalForeign);
  }, [foreignGovFee, foreignAgencyFee, foreignVat, foreignEtcFee, govFee, agencyFee, vat, etcFee, exchangeRate, setValue, getValues]);

  // [2] 합계 및 미수금 자동 계산
  useEffect(() => {
    const totalKRW = govFee + agencyFee + vat + etcFee;
  ...
  }, [govFee, agencyFee, vat, etcFee, depAmount, abandonAmount, setValue]);

  //   [신규] 원화 입력 시 외화 역산 로직 (onBlur 사용하여 입력 방해 최소화)
  const handleKRWBlur = (foreignName: keyof BillOutgoingFormInput, krwValue: string) => {
    lastModifiedCurrency.current = 'KRW';
    const krw = parseNumber(krwValue);
    if (!krw) return;
    const rate = exchangeRate || 1;
    // 외화 = 원화 / 환율 (소수점 2자리 반올림)
    ...
    >
      ...
      <RHF.FormField vertical gap={1}>
        ...
        <RHF.Input
          ...
          priceOnly
          decimalOnly
          // DB utb_cost_mst.krw_amount = numeric(15,0) — 정수부 최대 14자리 + 콤마 포함 18자
          maxLength={18}
          onFocus={(e) => {
            if (e.target.value === "0") setValue("govFeePayAmount", "");
          }}
          className="[&>div]:max-w-36!"
        />
      </RHF.FormField>
      ...
      >
        ...
        <RHF.FormField label="① 관납료" className="flex-row justify-between [&>div]:w-36 [&>div]:gap-2">
          <RHF.Input
            ...
            decimalOnly
            onFocus={(e) => {
              lastModifiedCurrency.current = 'FOREIGN';
              if (e.target.value === "0") setValue("foreignGovFee", "");
            }}
          />
          <RHF.Input
            control={control}
            ...
            onFocus={(e) => {
              lastModifiedCurrency.current = 'KRW';
              if (e.target.value === "0") setValue("govFee", "");
            }}
            onBlur={(e) => handleKRWBlur("foreignGovFee", e.target.value)}
          />
        </RHF.FormField>
        ...
        <RHF.FormField label="② 수수료" className="flex-row justify-between [&>div]:w-36 [&>div]:gap-2">
          <RHF.Input
            ...
            onFocus={(e) => {
              lastModifiedCurrency.current = 'FOREIGN';
              if (e.target.value === "0") setValue("foreignAgencyFee", "");
            }}
          />
          <RHF.Input
            control={control}
            ...
            onFocus={(e) => {
              lastModifiedCurrency.current = 'KRW';
              if (e.target.value === "0") setValue("agencyFee", "");
            }}
            onBlur={(e) => handleKRWBlur("foreignAgencyFee", e.target.value)}
          />
        </RHF.FormField>
        ...
        <RHF.FormField label="③ 부가세" className="flex-row justify-between [&>div]:w-36 [&>div]:gap-2">
          <RHF.Input
            ...
            onFocus={(e) => {
              lastModifiedCurrency.current = 'FOREIGN';
              if (e.target.value === "0") setValue("foreignVat", "");
            }}
          />
          <RHF.Input
            control={control}
            ...
            onFocus={(e) => {
              lastModifiedCurrency.current = 'KRW';
              if (e.target.value === "0") setValue("vat", "");
            }}
            onBlur={(e) => handleKRWBlur("foreignVat", e.target.value)}
          />
        </RHF.FormField>
        ...
        <RHF.FormField label="⑤ 기타비용" className="flex-row justify-between [&>div]:w-36 [&>div]:gap-2">
          <RHF.Input
            ...
            onFocus={(e) => {
              lastModifiedCurrency.current = 'FOREIGN';
              if (e.target.value === "0") setValue("foreignEtcFee", "");
            }}
          />
          <RHF.Input
            control={control}
            ...
            onFocus={(e) => {
              lastModifiedCurrency.current = 'KRW';
              if (e.target.value === "0") setValue("etcFee", "");
            }}
            onBlur={(e) => handleKRWBlur("foreignEtcFee", e.target.value)}
          />
        </RHF.FormField>
