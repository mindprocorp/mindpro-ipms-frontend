...
import { AppSearchModal } from "@pages/common/modal/app-search/AppSearchModal.tsx";
import { type SuccessOurRefData } from "@pages/common/modal/ourref/OurRefModal.tsx";
...
const DefaultInfo = (codeList: CodeListType) => {
  const { control, setValue, reset, watch, getValues, formState: { isDirty } } = useFormContext<BillIncomingFormInput>();

  // 1. 고객사 Seq 실시간 감시
  const currentCustomerSeq = useWatch({
    control,
    name: "customer.customerSeq" as any,
  });
  ...
  const selectedName = watch("customer.customerName" as any);
  ...
  // 4. [수정 핵심] 고객사가 '사용자에 의해' 변경될 때만 담당자 정보 초기화 및 이름 자동 매핑
  useEffect(() => {
    if (currentCustomerSeq) {
      // 이름 자동 매핑 (유효성 검사 통과용 - 타입 불일치 방지 위해 String 변환)
      const selected = codeList.customerCodeList.find(
        (c) => String(c.value) === String(currentCustomerSeq)
      );
      if (selected) {
        setValue("customer.customerName" as any, selected.label, { shouldValidate: true });
      }

      // 1) 폼이 이미 로드된 상태(isDirty)이고 2) 이전 값과 현재 값이 다를 때만 초기화 실행
      if (prevCustomerSeqRef.current !== currentCustomerSeq && isDirty) {
  ...
  }, [currentCustomerSeq, setValue, isDirty, codeList.customerCodeList]);
    ...
    Object.entries(data).forEach(([key, value]) => {
      if (value && typeof value === 'object' && !Array.isArray(value)) {
        Object.entries(value).forEach(([subKey, subValue]) => {
          ...
          // [방어] null/undefined일 경우 빈 문자열 세팅하여 Zod 에러 방지
          setValue(`${key}.${subKey}` as any, subValue ?? "", { shouldValidate: true });
        });
      } else {
        ...
        if (['applicantName', 'clientName', 'foreignAgentName'].includes(key)) {
          ...
          setValue(key as any, nameValue ?? "", { shouldValidate: true });
        } else {
  ...
  useEffect(() => {
    if (selectedName && codeList.customerCodeList) {
      ...
      if (selected) {
        setValue("customer.customerSeq" as any, selected.value, { shouldValidate: true });
  ...
  useEffect(() => {
    setValue("caseCategory.codeName" as any, "외국청구서");
    setValue("caseCategory.code" as any, "80");
  }, [setValue]);
    ...
    <>
      ...
      >
        ...
        <FlexBox>
          ...
          <RHF.FormSelect control={control} name="invClass.code" items={codeList.billEventCodeList} label="청구분류" />
          <RHF.FormSelect control={control} name="invType.code" items={codeList.invTypeCodeList} label="청구종류" />
          <RHF.FormDatePicker control={control} name="invDate" label="청구일" />
          <RHF.Input
            control={control}
            name="invNo"
            label="청구번호"
          ...
          <FlexBox className="justify-between">
            <RHF.FormField gap={2} className="pr-12">
              ...
              <RHF.Input control={control} name="regNo" label="등록번호" disabled placeholder="" />
               <RHF.Input control={control} name="country.code" label="국가코드" disabled placeholder="" />
                <RHF.Input control={control} name="country.codeName" label="국가명" disabled placeholder="" />
            </RHF.FormField>
{/*
            <RHF.FormField gap={2}>
              <RHF.Input control={control} name="grade" label="등급" className="w-14" align="center" disabled placeholder="" />
              <RHF.Input control={control} name="finalClaimsCount" label="독립항" className="w-14" align="center" disabled placeholder="" />
              <RHF.Input control={control} name="dependentClaims" label="종속항" className="w-14" align="center" disabled placeholder="" />
              <RHF.Input control={control} name="specPage" label="명세서" className="w-14" align="center" disabled placeholder="" />
              <RHF.Input control={control} name="drawingCount" label="도면수" className="w-14" align="center" disabled placeholder="" />
              <RHF.Input control={control} name="figureCount" label="도수" className="w-14" align="center" disabled placeholder="" />
            </RHF.FormField> */}
          </FlexBox>
        </UnitInnerBox>
      ...
      <AppSearchModal open={isOurRefOpenModal} onOpenChange={(o) => setIsOurRefOpenModal(o)} title="사건번호로 OurRef 선택" input={ourRefInputKeyInfo} onSuccess={onOurRefSuccess} />
      <CustomerSelectModal
        open={cstModal.open}
