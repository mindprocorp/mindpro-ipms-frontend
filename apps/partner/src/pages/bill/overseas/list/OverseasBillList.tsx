...
import type { BillIncomingListRequest } from "@shared/api/bill/billOverseasIncomingApi.ts";
...
const OverseasBillList = () => {
  const getBillListMutation = useMutation(billOverseasIncomingQueries.getBillIncomingList());
  ...
  const getBillList = (listParams: BillIncomingListRequest) => {
  ...
  useEffect(() => {
    const param: BillIncomingListRequest = {
      page,
      pageSize,
    };
    getBillList(param);
  ...
  const handleExcelDownload = async () => {
    ...
    if (response.data?.list) {
      downloadExcel(response.data.list, "overseas_bill_list", overseaBillColumnsData);
    }
  };
  ...
  return (
    <>
      <PageTitleArea className="pb-2" title="외국청구서(인커밍) 검색">
        <DataMenuButton data={listData} fileName="overseas_bill_list" columns={overseaBillColumnsData} onExcelClick={handleExcelDownload} />
        <Button variant="outline-pink" size="h28" onClick={() => navigate("/bill/overseas/write")}>
          <Icons.Plus />
          신규등록
        </Button>
{/* <Button
      ...
      <DataTable
        ...
        onRowClick={(_, rowData) => {
          ...
          if (id) {
            navigate(`/bill/overseas/detail/${id}`);
          }
        }}
      />
    </>
...
export default OverseasBillList;
