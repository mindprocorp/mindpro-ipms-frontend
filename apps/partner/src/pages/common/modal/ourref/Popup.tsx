...
import type { SearchAppItem } from "@shared/api/common/commApi";
import { MemberSearchBox, OurRefTag, NameTagCheck } from "./name-tag/OurRefTag.tsx";
import { useOurRefs } from "./model/useOurRefs.ts";
...
export const OrganizationPopup = ({ title, open, onOpenChange, onSuccess }: ModalProps) => {
  const { check, member, selectHandle, memFind } = useOurRefs();
      ...
      <div className="border-border-100 flex border-y">
        ...
        <CustomScrollArea className="h-100 w-full">
          <div className="flex-1">
            ...
            <div className="p-2">
              {member.map((item: SearchAppItem, index: number) => {
                return (
