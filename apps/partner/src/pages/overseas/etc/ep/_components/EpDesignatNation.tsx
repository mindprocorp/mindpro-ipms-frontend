...
const EpDesignatNation = () => {
  ...
  const handleCountrySelect = (selected: CountryData[]) => {
    ...
    const current = getValues("designatedStateInfo.designated");
    const merged = Array.from(new Set([...(Array.isArray(current) ? current : []), ...codes]));
    setValue("designatedStateInfo.designated", merged as any, { shouldValidate: true });
  };
  ...
  return (
    <FormUnitBox fullsize title="지정국가">
      <RHF.FormTextarea
        control={control}
        name="designatedStateInfo.designated"
        actions={
...
export default EpDesignatNation;
