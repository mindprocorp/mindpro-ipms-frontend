import { type FormInput } from "@pages/domestic/detail/DomesticForm.tsx";
...
const EpSasido = () => {
  const {
    ...
    control,
    formState: { errors },
  } = useFormContext<OverseasEpFormInput>();
  ...
  const deleteImageMutation = useMutation(overseasEpQueries.deleteEpImage());
  ...
  const FIELD_NAME = "mainDrawingFile";
  ...
  return (
    <FormUnitBox vertical title="대표도">
      ...
      <Label htmlFor="mainDrawingFile" className={cn("[&>input]:hidden", style)}>
      ...
      {errors.mainDrawingFile && (
        <p className="text-xs text-red-500">{errors.mainDrawingFile.message}</p>
      )}
