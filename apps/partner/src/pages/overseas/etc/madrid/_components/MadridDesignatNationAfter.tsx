...
const MadridDesignatNationAfter = () => {
  ...
  const handleCountrySelect = (selected: CountryData[]) => {
    ...
    const current = getValues("appStrategy.subsequent");
    const merged = Array.from(new Set([...(Array.isArray(current) ? current : []), ...codes]));
    setValue("appStrategy.subsequent", merged as any, { shouldValidate: true });
  };
...
export default MadridDesignatNationAfter;
