...
const MadridSasidoFile = ({ fileInfo }: Props) => {
  const {
    setValue,
    getValues,
    formState: { errors },
  } = useFormContext<OverseasMadridFormInput>();
  ...
  const deleteImageMutation = useMutation(overseasMadridQueries.deleteMadridImage());
  ...
  const FIELD_NAME = "trademarkImage";
  ...
  return (
    <FormUnitBox vertical title="상표이미지">
      ...
      <Label htmlFor="madridImageFile" className={cn("[&>input]:hidden", style)}>
      ...
      {errors.trademarkImage && (
        <p className="text-xs text-red-500">{errors.trademarkImage.message}</p>
      )}
...
export default MadridSasidoFile;
