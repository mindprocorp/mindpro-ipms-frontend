...
export const Modal24 = ({ title, open, onOpenChange, onSuccess }: ModalProps) => {
  ...
  return (
    <FormProvider {...form}>
      ...
      >
        <FlexBox vertical>
          <FlexBox>
            <RHF.FormDatePicker control={form.control} name="testVal" label="송금일" />
            <RHF.Input control={form.control} name="testVal" label="송금액" />
          </FlexBox>

          <FlexBox>
            <RHF.Input control={form.control} name="testVal" label="환율" />
            <RHF.Input control={form.control} name="testVal" label="환산비용" important />
          </FlexBox>

          <RHF.Input control={form.control} name="testVal" label="송금수수료" />
